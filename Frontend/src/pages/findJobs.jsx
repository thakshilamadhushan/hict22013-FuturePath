...
export default function FindJobs() {
	...
	const filtered = useMemo(() => {
		...
		return JOBS.filter((job) => {
			if (activeFilter !== 'All' && activeFilter !== job.type && !(activeFilter === 'Remote' && job.type === 'Remote')) {
				return false
			}
			if (q) {
			...
			if (loc) {
				if (!job.location.toLowerCase().includes(loc) && job.location.toLowerCase() !== 'remote') return false
			}
			return true
		})
	...
	return (
		<div className="find-jobs-page">
			<header className="fj-header">
				...
				<p className="fj-sub">AI-matched opportunities based on your profile — 12 roles available</p>
				...
				>
					...
					<button className="fj-search-btn" onClick={() => {}} aria-label="Search">
						Search
					</button>
				</form>
			</header>
					...
					<article key={job.id} className="fj-card">
						...
						<div className="fj-card-footer">
							<div className="fj-footer-left">
								{SHOW_MARKERS && <span className="fj-edit-dot" title="Edit salary" />}
								<div className="fj-salary">{job.salary}</div>
								{SHOW_MARKERS && <span className="fj-edit-dot" title="Edit type" />}
								<div className="fj-type">{job.type}</div>
							</div>
							<div className="fj-footer-right">
								{SHOW_MARKERS && <span className="fj-edit-dot" title="Edit match" />}
								<div className="fj-match">{job.match}% match</div>
								{SHOW_MARKERS && <span className="fj-edit-dot" title="Edit apply button" />}
